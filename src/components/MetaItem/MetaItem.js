...
const MetaItem = React.memo(({ className, type, name, poster, posterShape, posterChangeCursor, progress, newVideos, options, deepLinks, dataset, optionOnSelect, onDismissClick, onPlayClick, watched, titleAction, ...props }) => {
    ...
    const titleActionOnClick = React.useCallback((event) => {
        event.preventDefault();
        event.nativeEvent.selectPrevented = true;
        event.nativeEvent.buttonClickPrevented = true;
        if (typeof titleAction?.onClick === 'function') {
            titleAction.onClick(event);
        }
    }, [titleAction]);
    return (
            ...
            <div className={classnames(styles['poster-container'], { 'poster-change-cursor': posterChangeCursor })}>
                {
                    onDismissClick ?
                        <div title={t('LIBRARY_RESUME_DISMISS')} className={styles['dismiss-icon-layer']} onClick={onDismissClick}>
                            <div className={styles['dismiss-minus']} />
                        </div>
                        :
                        null
                }
            ...
            </div>
            {
                (typeof name === 'string' && name.length > 0) || (Array.isArray(options) && options.length > 0) ?
                    <div className={styles['title-bar-container']}>
                        {
                            titleAction ?
                                <Button className={classnames(styles['title-label'], styles['title-label-button'])} title={titleAction.title} tabIndex={-1} onClick={titleActionOnClick}>
                        ...
                        {
                            !titleAction && Array.isArray(options) && options.length > 0 ?
                                <Multiselect
